...
def build_everything(args: arg_util.Args):
    ...
    from models import VAR, VQVAE, build_vae_var_teacher_student
    from trainer import VARTrainer
    from utils.amp_sc import AmpOptimizer
    from utils.lr_control import filter_params
    
    vae_local, var_teacher, var_student = build_vae_var_teacher_student(
        V=4096, Cvae=32, ch=160, share_quant_resi=4,
        device=dist.get_device(), patch_nums=args.patch_nums,
        num_classes=num_classes, depth_teacher=20, depth_student=20, shared_aln=args.saln, attn_l2_norm=args.anorm,
    ...
    if dist.is_local_master():
        if not os.path.exists(vae_ckpt):
           print(f'[Error] VAE pretrained weights file does not exist: {vae_ckpt}')
    dist.barrier()
    vae_local.load_state_dict(torch.load(vae_ckpt, map_location='cpu'), strict=True)

    var_teacher_ckpt = 'path_to_var_teacher_checkpoint'
    var_student_ckpt = 'path_to_var_student_checkpoint'

    if dist.is_local_master():
        if not os.path.exists(var_teacher_ckpt):
            os.system(f'[Error] VAR pretrained weights file does not exist: {var_teacher_ckpt}')
            sys.exit(1)
        if not os.path.exists(var_student_ckpt):
            os.system(f'[Error] VAR pretrained weights file does not exist: {var_student_ckpt}')
            sys.exit(1)
    dist.barrier()

    print(f"[INFO] Loading VAR pretrained weights: {var_teacher_ckpt}")

    teacher_checkpoint = torch.load(var_teacher_ckpt, map_location='cpu')
    if 'trainer' in teacher_checkpoint:
        print("Detected training checkpoint file, extracting model weights...")
        if 'var_wo_ddp' in teacher_checkpoint['trainer']:
            model_weights = teacher_checkpoint['trainer']['var_wo_ddp']
            var_teacher.load_state_dict(model_weights, strict=True)
            print("Successfully extracted model weights from training checkpoint")

    else:
        print("Loading standard model weights file...")
        var_teacher.load_state_dict(teacher_checkpoint, strict=True)
    
    print(f"[INFO] VAR_teacher pretrained weights loading completed")

    student_checkpoint = torch.load(var_student_ckpt, map_location='cpu')
    if 'trainer' in student_checkpoint:
        print("Detected training checkpoint file, extracting model weights...")
        if 'var_wo_ddp' in student_checkpoint['trainer']:
            model_weights = student_checkpoint['trainer']['var_wo_ddp']
            var_student.load_state_dict(model_weights, strict=True)
            print("Successfully extracted model weights from training checkpoint")

    else:
        print("Loading standard model weights file...")
        var_student.load_state_dict(student_checkpoint, strict=True)
    
    print(f"[INFO] VAR_student pretrained weights loading completed")

    var_teacher.eval()
    for param in var_teacher.parameters():
        param.requires_grad = False
    var_teacher = var_teacher.to(dist.get_device())

    var_student_raw = var_student

    vae_local: VQVAE = args.compile_model(vae_local, args.vfast)
    var_student: VAR = args.compile_model(var_student, args.tfast)
    var_student: DDP = (DDP if dist.initialized() else NullDDP)(var_student, device_ids=[dist.get_local_rank()], find_unused_parameters=False, broadcast_buffers=False)
    
    print(f'[INIT] VAR_student model = {var_student}\n\n')
    ...
    print(f'[INIT][#para] ' + ', '.join([f'{k}={count_p(m)}' for k, m in (('VAR_student', var_student_raw),('VAR_teacher', var_teacher))]) + '\n\n')
    
    names, paras, para_groups = filter_params(var_student_raw, nowd_keys={
    ...
    trainer = VARTrainer(
        ...
        vae_local=vae_local, var_student=var_student_raw, var=var_student, var_teacher=var_teacher,
        var_opt=var_optim, label_smooth=args.ls,
    )
    if trainer_state is not None and len(trainer_state):
        trainer.load_state_dict(trainer_state, strict=True, skip_vae=True)
    del vae_local, var_student_raw, var_student, var_optim
    ...
    for ep in range(start_ep, args.ep):
        ...
        if is_val_and_also_saving:
            ...
            if dist.is_local_master():
                ...
                trainer.ema.store()
                trainer.ema.copy_to_model()
                
                ema_model_path = os.path.join(args.local_out_dir_path, 'ar-ema-model.pth')
                torch.save(trainer.var_wo_ddp.state_dict(), ema_model_path)
...
def train_one_ep(ep: int, is_first_ep: bool, start_it: int, args: arg_util.Args, tb_lg: misc.TensorboardLogger, ld_or_itrt, iters_train: int, trainer):
    ...
    for it, (inp, label) in me.log_every(start_it, iters_train, ld_or_itrt, 30 if iters_train > 8000 else 5, header):
        ...
        min_tlr, max_tlr, min_twd, max_twd = lr_wd_annealing(args.sche, trainer.var_opt.optimizer, args.tlr, args.twd, args.twde, g_it // args.ac, wp_it // args.ac, max_it // args.ac , wp0=args.wp0, wpe=args.wpe)
        ...
        grad_norm, scale_log2 = trainer.train_step(
            ...
            inp_B3HW=inp, label_B=label, prog_si=prog_si, prog_wp_it=args.pgwp * iters_train, max_it=max_it,
        )
