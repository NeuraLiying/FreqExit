...
class VectorQuantizer2(nn.Module):
    ...
    def get_next_autoregressive_input_inpaint(self, si: int, SN: int, f_hat: torch.Tensor, h_BChw: torch.Tensor, f_hats, mask_in, mask_out): 
        HW = self.v_patch_nums[-1]
        if si != SN-1:
            h = self.quant_resi[si/(SN-1)](F.interpolate(h_BChw, size=(HW, HW), mode='bicubic'))     
            f_hat = f_hat*mask_out + h*mask_out + f_hats[si]*mask_in
            return f_hat, F.interpolate(f_hat, size=(self.v_patch_nums[si+1], self.v_patch_nums[si+1]), mode='area')
        else:
            h = self.quant_resi[si/(SN-1)](h_BChw)
            f_hat = f_hat*mask_out + h*mask_out + f_hats[si]*mask_in
